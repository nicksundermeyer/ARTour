...
public class ThesisController : MonoBehaviour {
    ...
    [Serializable]
    public class PrefabDictionary : SerializableDictionary<string, ThesisVisualizer> { }

    // Dictionary of strings (image names from database) with prefabs
    [SerializeField]
    public PrefabDictionary Prefabs;

    private ThesisVisualizer currentObj;
    ...
    private Dictionary<int, ThesisVisualizer> m_Visualizers = new Dictionary<int, ThesisVisualizer> ();
    ...
    public void Update () {
        ...
        foreach (var image in m_TempAugmentedImages) {
            ThesisVisualizer visualizer = null;
            m_Visualizers.TryGetValue (image.DatabaseIndex, out visualizer);
            if (image.TrackingState == TrackingState.Tracking && visualizer == null) {
                // Create an anchor to ensure that ARCore keeps tracking this augmented image.
                Anchor anchor = image.CreateAnchor (image.CenterPose);
                Prefabs.TryGetValue (image.Name, out currentObj);
                visualizer = (ThesisVisualizer) Instantiate (currentObj, anchor.transform);
